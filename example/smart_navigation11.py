...
#
# This program controls a small autonomous car.
# It uses ultrasonic sensors to scan its surroundings, builds an occupancy grid map,
# uses a camera with AI object detection to spot stop signs and traffic lights,
# and navigates toward a preset goal position.
#
# The code is organized into two main classes:
# 1. EnhancedMapper: Handles environment scanning, mapping, and visualization.
# 2. TeamIoT_SmartNavigator: Handles camera detection, navigation planning (using A*), and driving commands.
...
# EnhancedMapper: Scans the environment and builds an occupancy grid map.
class EnhancedMapper:
    def __init__(self, px: Picarx):
        ...
        self.heading = 135                # Initial direction the car is facing (degrees).
...
# TeamIoT_SmartNavigator: Main class for navigation and driving control.
class TeamIoT_SmartNavigator:
    def __init__(self):
        ...
        self.CELL_SIZE_CM = 2
        self.position = (self.map_size - 10, 10)  # Starting cell.
        self.heading = 135
    ...
    def pivot_turn(self, direction, duration=1.2, forward=True):
        """
        Performs a sharp pivot turn (in-place turn) to avoid obstacles.
        After turning, the car rescans its environment.
        """
        if direction == "left":
            angle = -40  # Left turn angle.
            angle_change = -45
